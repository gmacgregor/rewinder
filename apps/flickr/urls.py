...
urlpatterns = patterns('django.views.generic.date_based',
    ...
    #url(r'^(?P<year>\d{4})/(?P<month>\d{2})/(?P<day>\d{2})/(?P<slug>[-\w]+)/$', 'object_detail', dict(photo_dict, month_format='%m'), name='photo_detail'),
    ...
    url(r'^$', 'archive_index', dict(photo_dict, num_latest=5), name='photo_list'),
)